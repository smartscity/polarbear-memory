...
const server = readFileSync(resolve(root, "src/protocol-local/server.ts"), "utf8");
const body = server.match(/export const ADMIN_CAPABILITIES = \[([\s\S]*?)\] as const;/)?.[1] ?? "";
...
if (!server.includes(`export const ADMIN_API_VERSION = "${contract.version}";`)) {
