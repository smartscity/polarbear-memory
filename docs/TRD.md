...
Admin API 1.2 adds `memories.record/complete/feedback` and `usage.token_savings/token_savings_reset` to the 1.1 browsing and governance surface. Its stable capabilities are `projects.status/diagnostics/config/config_update`, `memories.list/get/record/history/update/verify/archive/restore/complete/feedback/relate/purge_preview/purge`, `contexts.explain`, `usage.token_savings/token_savings_reset`, `maintenance.preview/run`, `backups.list/create/verify/restore_preview/restore`, `knowledge.promote_preview/promote`, and `system.shutdown`. `memories.record` accepts all nine V2 Knowledge types, temporal validity, file anchors, Evidence IDs, and Entities; the Engine still owns all business validation and transactions. Token-savings reset requires the exact confirmation `RESET` and only starts a new measurement period. List and detail operations return DTOs without database paths or table details. Promote uses a preview SHA-256 for two-phase confirmation and exclusive file creation. Purge and database restore also require preview plus exact confirmation. Restore uses the cooperative maintenance lock and fails while active client leases exist.

The Engine owns the Admin contract in `api/admin-v1.json` and the canonical `api/admin-v1.types.ts`. Desktop generates its TypeScript capabilities and DTOs from the vendored contract. Drift gates cover the Engine implementation, complete DTOs and enums, the Desktop Rust allowlist, API version, and generated output. Cross-repository integration checks must compare both capability contracts and DTO sources before running the real Rust proxy → Node Engine → SQLite test.
...
Implementation status (v0.0.5 / v0.1 Admin Console): the runnable engineering loop is complete. Admin API 1.2 provides creation and filtering for all nine V2 Knowledge types, temporal validity, file anchors, Evidence and Entity data, every relation type, task completion and cancellation, feedback, and token-savings inspection and reset. It retains the 1.1 revision, audited edit, verification, archive and restore, explicitly approved purge, Context Explain, capture and retention configuration, maintenance preview and apply, redacted diagnostics, consistent backup creation, verification and restore, and Engine service lifecycle features. Timeline is the searchable and filterable list ordered by descending `updated_at`; Forget maps to recoverable archive. Cross-process client leases and the maintenance lock protect Restore. Desktop native state binds the current canonical workspace, while canonical DTO, capability, and allowlist drift gates prevent silent cross-repository protocol divergence. Whether users continue to use the Viewer, improve correction quality, or create more durable knowledge still requires product trials and cannot be proven by automated tests.
